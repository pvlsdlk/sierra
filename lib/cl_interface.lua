...
function meta:Interface()
	self.Class = function(pnl, name, ...)
		local class = classes[name]
		assert(class, "[Interface]: Class "..name.." does not exist.")
...
function Interface(c, p, n)
	local pnl = vgui.Create(c, p, n)
	return pnl:Interface()
end

-- Returns a single cached copy of a material or creates it if it doesn't exist.
function Sierra.GetCachedMaterial(materialPath)
	-- Cache the material.
	Sierra.cachedMaterials = Sierra.cachedMaterials or {}
	Sierra.cachedMaterials[materialPath] = Sierra.cachedMaterials[materialPath] or Material(materialPath)

	return Sierra.cachedMaterials[materialPath]
end

function Sierra.BlurPanel(panel, layers, density, alpha)
